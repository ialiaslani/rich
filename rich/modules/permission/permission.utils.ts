export class PermissionUtils {
  ...
  comparePermission(permission: string, pattern: string) {
    ...
    while (true) {
      //POINT TO CHARECTERS
      const permissionChar = permissionChars[permissionPointer];
      const patternChar = patternChars[patternPointer];
      ...
      if (permissionChar === patternChar) {
        ...
        else if (permissionPointer >= permissionLength || patternPointer >= patternLength) return false;
        continue;
      }
  ...
  generatePermission(allRoutes: string[], selectedRoutes: string[]) {
    ...
    const patterns = [];
    const hasOnes = [false, false, false, false];
  ...
  mapPermissions(i, m) {
    if (i === 0) return m.split('.')[i];
    else if (i === 1) return m.split('.')[i] + '.' + m.split('.')[i - 1];
    else if (i === 2) return m.split('.')[i] + '.' + m.split('.')[i - 1] + '.' + m.split('.')[i - 2];
    else return m;
  }
}
